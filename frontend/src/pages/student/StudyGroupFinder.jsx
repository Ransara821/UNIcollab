...
import {
  getStudyGroups, createStudyGroup, getMyGroup, toggleGroupStatus, updateSkillsNeeded,
...
export default function StudyGroupFinder() {
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      fetchProfile(); fetchSuggestions(); fetchPool();
      ...
      <div className={`bg-white rounded-2xl border ${isSuggestion ? 'border-emerald-200' : 'border-slate-100'} shadow-sm p-5 hover:shadow-md transition-all flex flex-col`}>
        <div className="flex items-start justify-between mb-2">
          ...
          <div className="flex flex-col items-end gap-1 ml-2 shrink-0">
            <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${g.status === 'open' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{g.status}</span>
        ...
        {g.description && <p className="text-xs text-slate-500 mb-3 line-clamp-2">{g.description}</p>}
        <div className="flex flex-wrap gap-1 mb-3">
          {(g.requiredSkills || []).slice(0, 4).map(sk => (
            <span key={sk} className="text-xs bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full">{sk}</span>
          ))}
          {(g.requiredSkills || []).length > 4 && <span className="text-xs bg-slate-100 text-slate-400 px-2 py-0.5 rounded-full">+{g.requiredSkills.length - 4}</span>}
        </div>
        {g.skillsNeeded && (
          <div className="text-xs bg-orange-50 text-orange-700 border border-orange-100 rounded-lg px-3 py-2 mb-3">🔧 Needs: {g.skillsNeeded}</div>
        )}
        <div className="flex gap-3 text-xs text-slate-400 mt-auto mb-3">
          <span>👥 {(g.members || []).length}/{g.maxSize}</span>
        ...
        {isLeader  && <p className="text-center text-xs text-orange-500 font-bold">👑 Your Group</p>}
        {isMember && !isLeader && <p className="text-center text-xs text-emerald-500 font-semibold">✓ Member</p>}
      </div>
    );
  };
  ...
  const AvailGrid = () => (
    <div className="overflow-x-auto">
      <table className="w-full text-xs">
        <thead>
          <tr>
            <th className="text-left text-slate-400 w-16 pb-2" />
            {SLOTS.map(s => <th key={s} className="text-slate-500 font-semibold pb-2 px-1 capitalize text-center">{s}</th>)}
          </tr>
        </thead>
        <tbody>
          {DAYS.map(day => (
            <tr key={day}>
              <td className="text-slate-500 font-semibold pr-2 py-1 capitalize">{day.slice(0,3)}</td>
              {SLOTS.map(slot => {
                const on = (pf.availability[day] || []).includes(slot);
                return (
                  <td key={slot} className="py-1 px-1 text-center">
                    <button onClick={() => toggleAvail(day, slot)}
                      className={`w-9 h-7 rounded-lg text-xs font-bold transition-all ${on ? 'bg-emerald-500 text-white' : 'bg-slate-100 text-slate-300 hover:bg-slate-200'}`}>
                      {on ? '✓' : ''}
                    </button>
                  </td>
                );
  ...
  return (
    <div className="p-6 bg-slate-50 min-h-screen">
      <div className="mb-5">
        ...
        <p className="text-slate-500 text-sm mt-1">Find your perfect study group or get matched by AI</p>
      </div>
        ...
        {TABS.map(t => (
          <button key={t.id} onClick={() => setActiveTab(t.id)}
            className={`flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-sm font-bold whitespace-nowrap transition-all ${
              activeTab === t.id ? 'bg-emerald-500 text-white shadow-md' : 'bg-white text-slate-600 hover:bg-slate-100 border border-slate-200'
            }`}>
            {t.icon} {t.label}
          </button>
        ))}
          ...
          {suggestions.length > 0 && (
            <div className="mb-8">
              <div className="flex items-center gap-2 mb-3">
                <h2 className="text-base font-extrabold text-slate-900">🤖 AI Suggestions for You</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {suggestions.slice(0, 3).map(g => <GroupCard key={g._id} g={g} isSuggestion />)}
              </div>
        ...
        <div className="max-w-3xl">
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
            <div className="flex items-center gap-2 mb-6">
              ...
              <span className="text-xs font-semibold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full">Powers AI matching</span>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between bg-orange-50 border border-orange-100 rounded-xl px-4 py-3">
                  <div>
                    <p className="text-sm font-bold text-orange-800">SOS Flag 🆘</p>
                    <p className="text-xs text-orange-500">Urgent — bumps you to top of pool</p>
                  </div>
                  <button onClick={() => setPf(p => ({ ...p, sosFlag: !p.sosFlag }))}
                    className={`relative w-11 h-6 rounded-full transition-colors ${pf.sosFlag ? 'bg-orange-500' : 'bg-slate-200'}`}>
                    <span className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-all ${pf.sosFlag ? 'left-5' : 'left-0.5'}`} />
                  </button>
                </div>
              </div>
              <div className="space-y-4">
                ...
                <div>
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wide block mb-2">Availability</label>
                  <AvailGrid />
                </div>
              </div>
            </div>
            <div className="mt-6 pt-5 border-t border-slate-100">
              <button onClick={handleSaveProfile} disabled={actLoad.saveProfile}
                className="w-full py-3 bg-emerald-500 text-white rounded-xl font-bold text-sm hover:bg-emerald-600 disabled:opacity-50 transition-all">
                {actLoad.saveProfile ? 'Saving...' : '💾 Save Profile & Recompute Vectors'}
              </button>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          {loading.pool ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {pool.map(student => (
                <div key={student._id} className={`bg-white rounded-2xl border ${student.sosFlag ? 'border-orange-200' : 'border-slate-100'} shadow-sm p-5`}>
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-400">Year {student.year} · {student.faculty || 'N/A'}</p>
                      </div>
                    </div>
                    <div className="flex flex-col items-end gap-1">
                      {student.sosFlag && <span className="text-xs font-bold bg-orange-500 text-white px-2 py-0.5 rounded-full">🆘 SOS</span>}
                      <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${
                        student.urgencyScore >= 50 ? 'bg-red-100 text-red-700' : student.urgencyScore >= 25 ? 'bg-orange-100 text-orange-700' : 'bg-slate-100 text-slate-500'
                      }`}>⚡ {student.urgencyScore}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-3 text-xs text-slate-400 mb-3">
                    <span>🧠 {student.workingStyle}</span>
                    {student.deadline && <span>📅 {new Date(student.deadline).toLocaleDateString()}</span>}
                  </div>
                  {myGroupRole === 'leader' && myGroup?.status === 'open' && student.userId !== uid(user) && (
                    <button onClick={() => handleInvite(student.userId)} disabled={actLoad[`inv_${student.userId}`]}
                      className="w-full py-2 bg-emerald-500 text-white rounded-xl text-xs font-bold hover:bg-emerald-600 disabled:opacity-50 transition-all">
                      {actLoad[`inv_${student.userId}`] ? '...' : '✉️ Invite to My Group'}
                    </button>
                  )}
                </div>
              ))}
              ...
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex flex-col items-end gap-1">
                    ...
                    <span className="text-xs font-bold text-orange-500">{myGroupRole === 'leader' ? '👑 Leader' : '👤 Member'}</span>
                  </div>
                </div>
                  ...
                  {(myGroup.members || []).map(m => (
                    <div key={m.userId} className="flex items-center gap-3 p-2 rounded-xl hover:bg-slate-50">
                      ...
                      <div>
                        ...
                        {m.userId === myGroup.leader && <p className="text-xs text-orange-500 font-semibold">👑 Leader</p>}
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {myGroupRole === 'leader' && (
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
                  ...
                  {groupReqs.length === 0 ? (
                    ...
                    <div className="space-y-3">
                      {groupReqs.map(req => (
                        <div key={req._id} className="border border-slate-100 rounded-xl p-3 flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            <div className="flex items-center gap-2">
                              <div className="w-7 h-7 bg-slate-100 rounded-full flex items-center justify-center text-slate-600 font-bold text-xs">
                                {(req.studentName || 'S')[0].toUpperCase()}
                              </div>
                              <p className="font-bold text-slate-800 text-sm">{req.studentName || 'Student'}</p>
                            </div>
                            <p className="text-xs text-slate-400 mt-0.5 ml-9">{req.studentEmail}</p>
                            {req.message && <p className="text-xs text-slate-500 italic mt-1 ml-9">"{req.message}"</p>}
                            <p className="text-xs text-slate-300 mt-1 ml-9">{new Date(req.createdAt).toLocaleDateString()}</p>
                          </div>
                            ...
                            {req.status !== 'pending' ? (
                              ...
                              <>
                                <button onClick={() => handleLeaderAction(req._id, 'accepted')} disabled={actLoad[req._id]}
                                  className="text-xs bg-emerald-500 text-white px-3 py-1.5 rounded-lg font-bold hover:bg-emerald-600 disabled:opacity-50">Accept</button>
                                <button onClick={() => handleLeaderAction(req._id, 'waitlisted')} disabled={actLoad[req._id]}
                                  className="text-xs bg-blue-50 text-blue-600 px-3 py-1.5 rounded-lg font-bold hover:bg-blue-100 disabled:opacity-50">Waitlist</button>
                                <button onClick={() => handleLeaderAction(req._id, 'declined')} disabled={actLoad[req._id]}
                                  className="text-xs bg-red-50 text-red-500 px-3 py-1.5 rounded-lg font-bold hover:bg-red-100 disabled:opacity-50">Decline</button>
                              </>
                            )}
                          </div>
                        </div>
